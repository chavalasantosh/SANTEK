...
class WeaviateVectorStore:
    ...
    def _convert_filter(self, filter_dict: Dict) -> Optional[Any]:
        ...
        # Build Weaviate filter using Filter class
        # Weaviate uses a chainable filter API
        conditions = []
        ...
        for key, value in filter_dict.items():
            if isinstance(value, dict):
                ...
                if "$eq" in value:
                    conditions.append(Filter.by_property(key).equal(value["$eq"]))
                elif "$ne" in value:
                    conditions.append(Filter.by_property(key).not_equal(value["$ne"]))
                elif "$in" in value:
                    # Weaviate uses .contains_any() for $in
                    conditions.append(Filter.by_property(key).contains_any(value["$in"]))
                elif "$nin" in value:
                    # Weaviate doesn't have direct $nin, use not_contains_any
                    conditions.append(Filter.by_property(key).not_contains_any(value["$nin"]))
                elif "$gt" in value:
                    conditions.append(Filter.by_property(key).greater_than(value["$gt"]))
                elif "$gte" in value:
                    conditions.append(Filter.by_property(key).greater_or_equal(value["$gte"]))
                elif "$lt" in value:
                    conditions.append(Filter.by_property(key).less_than(value["$lt"]))
                elif "$lte" in value:
                    conditions.append(Filter.by_property(key).less_or_equal(value["$lte"]))
            else:
                # Simple equality
