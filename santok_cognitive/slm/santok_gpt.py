...
try:
    ...
    try:
        from santok_complete.src.core.core_tokenizer import tokenize_text
        SANTOK_TOKENIZER_AVAILABLE = True
    except ImportError:
...
class SanTOKLGM:
    ...
    def generate(self, prompt: str, max_tokens: int = 100, temperature: float = 0.8, repetition_penalty: float = 1.2) -> str:
        ...
        for _ in range(max_tokens):
            ...
            try:
                ...
                if repetition_penalty > 1.0 and len(generated_ids) > 0:
                    # Get recent tokens (last 10 tokens)
                    recent_tokens = generated_ids[-10:] if len(generated_ids) >= 10 else generated_ids
