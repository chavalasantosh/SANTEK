...
class ChromaVectorStore(SanTOKVectorStore):
    ...
    def add_tokens(
        ...
        with warnings.catch_warnings():  # pylint: disable=redefined-outer-name
    ...
    def _convert_filter(self, filter_dict: Dict) -> Optional[Dict]:
        """Convert simple filter dict to ChromaDB where format."""
        if not filter_dict:
            return None

        # ChromaDB uses a simple dict format for where clauses
        # Format: {"field": "value"} or {"field": {"$eq": "value"}}
        # Support common operators: $eq, $ne, $gt, $gte, $lt, $lte, $in, $nin
        chroma_filter = {}

        for key, value in filter_dict.items():
            if isinstance(value, dict):
                # Already in ChromaDB format with operators
                chroma_filter[key] = value
            else:
                # Simple equality - convert to explicit $eq format
...
class FAISSVectorStore(SanTOKVectorStore):
    ...
    def add(
        ...
        class DummyToken:  # pylint: disable=missing-class-docstring
    ...
    def add_tokens(
        ...
        self.index.add(embeddings)  # type: ignore[call-arg]  # pylint: disable=no-value-for-parameter
    ...
    def _apply_filter(self, results: List[Dict], filter_dict: Dict) -> List[Dict]:
        """Apply post-filtering to FAISS results (FAISS doesn't support native filtering)."""
        if not filter_dict:
            return results
        ...
        for result in results:
            ...
            for key, value in filter_dict.items():
                # Support simple equality and common operators
                if isinstance(value, dict):
    ...
    def search(
        ...
        # FAISS doesn't support native filtering, so we search more results
        # and filter afterwards if a filter is provided
        search_k = top_k * 10 if filter else top_k  # Search more if filtering
        search_k = min(search_k, self.index.ntotal)  # Can't search more than available

        # Search
        distances, indices = self.index.search(query_embedding, search_k)  # type: ignore[call-arg]  # pylint: disable=no-value-for-parameter
        ...
        # Apply post-filtering if filter provided
        if filter:
